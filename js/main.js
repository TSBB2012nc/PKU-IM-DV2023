...
function showPage1() {
  ...
  d3.select("#page")
    ...
    .attr("class", "footer")
    .text(
      "Data Visualization, 2023 Spring, Liang Hu, Jiaqi Lei, Haoyang Wang, Xin He"
    );
  // draw
  ...
  data = d3.csv("data/dots.csv").then((result) => {
    ...
    const globe = Globe()
      ...
      .ringRepeatPeriod("repeatPeriod")(document.getElementById("globeViz"));
...
function showPage2() {
  ...
  addLogo();
  // move logo
  d3.select("div.title").attr(
    "style",
    "top:5%;right:10%;left: auto;height: 100rem;"
  );
  ...
  function draw_map(start_year, end_year) {
    ...
    d3.csv("data/map.csv").then(function (data) {
        ...
        if (country != "") {
          ...
          if (flag == 0) {
            country_count.push({
              country: country,
              count: 1,
              info: [{ name: name, field: field, year: year, link: link }],
            });
          }
        }
      }
        ...
        .on("click", function (e) {
          ...
          if (!tooltip.empty()) {
            ...
            for (var i = 0; i < info_list.length; i++) {
              ...
              var tooltipAvatar = sub_tooltip
                .append("img")
                .attr("class", "laureate avatar");
              var tooltipName = sub_tooltip
                .append("p")
                .attr("class", "laureate name");
              var tooltipYear = sub_tooltip
                .append("p")
                .attr("class", "laureate year");
              var tooltipField = sub_tooltip
                .append("p")
                .attr("class", "laureate field");
              // 更新浮窗内容
      ...
      map_svg.call(
        zoom.transform,
        d3.zoomIdentity.scale(0.5).translate(350, 400)
      );
    });
  }
  ...
  var drag_left = d3
    ...
    .on("end", function (e) {
      d3.select(this).attr("stroke", "black");
      year = Math.round(
        time_ticks_scale.invert(
          Number(d3.select(this).attr("x")) + Number(0.5 * rect_width)
        )
      );
      start_year = year;
      // 重新调用绘图函数
    ...
    d3.csv("data/words_count.csv").then(function (data) {
      ...
      for (var i = 0; i < data.length; i++) {
        ...
        if (year >= start_year && year <= end_year) {
          ...
          if (field == "physics") {
            phy_list.push(data[i]);
          } else if (field == "chemistry") {
            che_list.push(data[i]);
          } else {
            med_list.push(data[i]);
          }
        }
      }
      ...
      if (med_list.length > limit) {
        ...
        var sorted_med_list = med_list;
      }

      // 合并三个领域的前limit个高频词
      var result_list = sorted_phy_list.concat(
        sorted_che_list,
        sorted_med_list
      );
...
function showPage3() {
  ...
  function drawBlock(blockId, blockCategory, blockData, blockText, color) {
    ...
    block
      .append("svg")
      .attr("id", blockCategory)
      .attr("height", 0)
      .attr("width", 0)
      .attr("class", "graph")
      .on("mouseover", "hoverBlock(0)")
      .on("mouseover", "clearAnimation()");
    ...
    d3.csv("data/" + blockData).then(function (data) {
      ...
      const scale2 = d3.scaleLinear().domain([1, 56]).range([1, 8]);
      ...
      const circles = svg
        ...
        .on("mouseout", function () {
          d3.select(this).style("opacity", 0.5); // 鼠标离开时将圆的透明度设为0，隐藏标签
...
function showPage4() {
  ...
  d3.select("#page")
    .append("iframe")
    .attr("src", "./network/triple_colab.html")
...
function showPage5() {
  ...
  ending
    .append("p")
    .attr("class", "end")
    .text("A hundred vessels contend, a thousand sails compete,")
    .append("p")
    .attr("class", "end")
    .text("as mankind's ceaseless march towards truth knows no retreat.")
    .append("p")
    .attr("class", "end")
    .text("The history of Nobel yet unfolds, with tales yet to be complete.");
}
